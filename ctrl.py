...
class ctrlAction(QThread):
    ...
    sig_checkFirmware = pyqtSignal(str)
    ...
    def checkFirmware(self):
        ...
        while 1:
            ...
            if endTime-startTime>2:
                self.checkFirmwareBool=False
                self.checkFirmwareMsg=""
                # self.emit(SIGNAL("sig_checkFirmware"),"false")
                self.sig_checkFirmware.emit("false")
                return
        if self.checkFirmwareMsg.find("Traceback")>=0 or self.checkFirmwareMsg.find("... ")>=0:
            ...
            # self.emit(SIGNAL("sig_checkFirmware"),self.checkFirmwareMsg)
            self.sig_checkFirmware.emit(self.checkFirmwareMsg)
            self.checkFirmwareMsg=""
            self.checkFirmwareBool=False
            return
    ...
    def downloadFile(self,filename):
        ...
        if afile[0:4] == './sd':
            afile = afile[1:]
        print("after downloadFileBool: filename: %s"%afile)
        ...
        if self.dragDropModel == True:
            ...
            print("downloadFile: linux")
            fileHandle=open(currentTempPath+"/"+str(filename.split("/")[-1]))
        elif sys.platform=="win32" and filename.find(":")<0:
            fileHandle=open(currentTempPath+filename,'rbU')
        elif sys.platform=="darwin" and filename.find(rootDirectoryPath)<0:
            fileHandle=open(currentTempPath+filename,'rbU')
        else:
            print(currentTempPath+"/"+str(filename.split("/")[-1]))
        ...
        print("sig_uiRecvFromCtrl emitted")
        ##################
        #open(filename,'w')
        ##################
        self.downloadFileMsg=""
        ...
        if self.currentBoard=="microbit":
            ...
            if afile[0:4] == './sd':
                afile = afile[1:]
            print("afile before open: %s"%str(afile))
            print("send to queue ctrltouart:::myfile=open(\'%s\',\'w\')\r\n"%str(afile))
    ...
    def getFileTree(self,dir):
        print("get file tree of %s"%dir)
        self.reflushTreeMsg=""
        self.ctrltouartQueue.put("ctrltouart:::os.listdir(\'%s\')\r\n"%dir)
        result=self.treeWaitUart()
        print("treeWaitUart result %s"%result)
        if result=="err":
            return result
