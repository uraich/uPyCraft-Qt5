...
#thread
        ...
        # self.connect(self.ctrl,SIGNAL("sig_checkFirmware"),self.checkFirmware)
        ...
        self.ctrl.sig_checkFirmware.connect(self.checkFirmware)
...
#create toolBars
    def createToolBars(self):
        ...
        fileToolBar.setStyleSheet("""QToolBar {background-color: orange ;spacing:8px;}""")
    ...
    def createBasicConfig(self):
        ...
        if not os.path.exists("%s/.uPyCraft/config.json"%rootDirectoryPath):
            ...
            configFile.write("{'serial':'/dev/ttyUSB0',\
                         'updateURL':'https://raw.githubusercontent.com/uraich/uPyCraft-Qt5/master/dist/uPyCraft.json',\
            ...
            if jsonDict.get('serial')==None or \
               ...
               jsonDict.get('workSpace')==None:
                configFile=open("%s/.uPyCraft/config.json"%rootDirectoryPath,'w')
                configFile.write("{'serial':'None','updateURL':'https://raw.githubusercontent.com/uraich/uPyCraft-Qt5/master/dist/uPyCraft.json',\
...
#File
    def slotOpenFile(self):
        ...
        filename=filename[0]
        print("slotOpenFile: filename[0]: %s"%filename)
    ...
    def slotSyntaxCheck(self):
        ...
        print("Syntax check on file %s"%syntaxCheckFilePath)
        ...
        pyflakesSyntaxCheck(str(syntaxCheckFilePath))
        ...
        self.terminal.append("Syntax check finished.")
    ...
    def slotDownloadFile(self):
        ...
        if self.fileName=="untitled":
            ...
            self.inDownloadFile=False
            return False
        print("File to download: %s"%self.fileName)
        print("DirectoryPath: %s"%rootDirectoryPath)
        if sys.platform=="linux" and str(self.fileName).find(rootDirectoryPath)>=0:
            afile=self.fileName
            print("Downloading file%s"%afile)
            self.terminal.append('file name: ' + afile)
            ...
            afile=self.fileName
            pyFileName=afile.split('/')[-1]
            shutil.copyfile(afile,str(currentTempPath+pyFileName))
            print("pyFileName: %s"%pyFileName)
            print("currentTempPath: %s"%currentTempPath)
            myfile=open(afile,'w',encoding='utf-8')
...
############Tools->InitConfig
    def slotInitConfig(self):
        ...
        path=os.getcwd()
        path=path.replace("\\","/")
        # remove the link to the workspace in .uPyCraft
        os.remove('%s/.uPyCraft/workSpace'%rootDirectoryPath)
        configFile=open("%s/.uPyCraft/config.json"%rootDirectoryPath,'w')
        configFile.write("{'serial':'None','updateURL':'https://raw.githubusercontent.com/uraich/uPyCraft-Qt5/master/dist/uPyCraft.json',\
...
###########Tools->Preferences
    ...
    def slotLanlocLocation(self,item):
        if self.preferencesDialog.landlocation.locationComboBox.currentText()=="China Mainland":
            ...
            jsonDict['updateURL']="https://raw.githubusercontent.com/uraich/uPyCraft-Qt5/master/dist/uPyCraft.json"
...
#####
    def pcOpenFile(self,filename):
        ...
        print("filename before open: %s"%filename)
    ...
    def updateFirmware(self,isAuto=False):
        ...
        if self.canNotIdentifyBoard:
            ...
            boardIndex=self.updateBin.boardComboBox.findText(self.currentBoard,Qt.MatchExactly)
            #print("boardIndex"+str(boardIndex))
            if boardIndex > 0:
        ...
        if self.updateBin.radioUPY.isChecked():
            ...
            if isCheckFirmware:
                ...
                try:
                    ...
                    self.terminal.append("Please re-open the uPy.")
            ...
            if self.updateBin.firmwareName.text()!="":
                ...
                if not os.path.exists(userFirmwareName):
                    self.terminal.append("user choosen firmware file does not exist!")
                    return
                ...
                print("size if micro python code: 0x%x"%userFirmwareSize)
